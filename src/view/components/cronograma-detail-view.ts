 ...
 *
 * Componente responsável por exibir os detalhes de um cronograma específico,
 * incluindo suas informações gerais e a lista de atividades associadas.
 *
 * Recursos:
 * - Mostra título, descrição, período e status do cronograma
 * - Lista todas as atividades com status visual (Concluída, Em andamento, etc.)
 * - Permite exclusão de atividades individualmente
 *
 * Elementos esperados no DOM:
 * - #cronograma-detail-screen → container principal onde o conteúdo será renderizado
 *
 * Interações:
 * - Botão "Voltar" para retornar à lista de cronogramas
 * - Botão "Excluir" em cada atividade
 * - Cálculo automático de status e dias restantes
 */
export class CronogramaDetailView {
  /** Elemento principal que contém a view de detalhes */
  public element: HTMLElement;

  /** Callback disparado ao excluir uma atividade */
  ...
  public showCronogramaDetails(cronograma: {
    ...
    // Cálculo do status do cronograma
    const hoje = new Date();
          ...
          <div class="atividade-card rounded-xl border border-gray-200 bg-white p-4 shadow-sm hover:shadow-md transition-shadow">
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                ...
                ${atividade.descricao ? `<p class="mt-1 text-sm text-gray-600">${atividade.descricao}</p>` : ''}
      ...
      <div class="cronograma-overview">
        <div class="cronograma-info rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <div class="cronograma-timeline mt-4 grid gap-2 text-sm">
            <div><strong>Período:</strong> ${inicio.toLocaleDateString('pt-BR')} – ${fim.toLocaleDateString('pt-BR')}</div>
            ...
            <div><strong>Atividades:</strong> ${cronograma.atividades ? cronograma.atividades.length : 0} no total</div>
          </div>
